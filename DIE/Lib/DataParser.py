...
class DataParser():
    ...
    def set_plugin_path(self, plugin_path):
        ...
        self.logger.info("[DIE] Plugin path is set to %s", plugin_path)

    def loadPlugins(self):
        """
        Load\\Reload all plugins found in the plugin location.
        """
        self.logger.info("[DIE] Loading Plugins from %s", self.pluginLocation)
        self.pManager.collectPlugins()

        all_plugins = self.pManager.getAllPlugins()
        if len(all_plugins) == 0:
            idaapi.msg("[DIE] Warning - No Plugins were loaded!\n")
            self.logger.error("[DIE] No plugins were loaded")

        for pluginInfo in all_plugins:

            # TODO: Validate plugins!
            self.logger.info("[DIE] Loading plugin %s", pluginInfo.name)
...
idaapi.msg("[DIE] Loading data parsers\n")
